...
export class AuhtService {
    ...
    public async loginProfesor(loginDto: LoginProfesorDto) {
        const { usuario } = prisma;

        const existProfesor = await usuario.findUnique({ where: { correo: loginDto.correo } });
        if (!existProfesor) throw RequestError.badRequest("Correo/Contraseña no valida");

        const isMatch = bcryptjsAdapter.compare(loginDto.passsword, existProfesor.password);
        if (!isMatch) throw RequestError.badRequest("Correo/Contraseña no valida");

        const token = await JwtAdapter.generateToken({ id: existProfesor.matricula });
        if (!token) throw RequestError.internalServerError();

        const { password, ...data } = existProfesor
        return {
            user: data,
            token,
        }
